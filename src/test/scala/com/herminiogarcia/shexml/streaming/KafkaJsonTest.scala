...
class KafkaJsonTest extends AnyFunSuite with RDFStatementCreator with ResultRetriever with KafkaTestDataGenerator {

  private val prefix = "http://example.com/"
  private val uniqueTopicId = "filmsJson1" + UUID.randomUUID().toString.replaceAll("-", "")
  private val kafkaEmitter = new KafkaTestDataEmitter("localhost:9092", uniqueTopicId)
  private val streamLine = Some(s"STREAM films_stream <kafka: localhost:9092 -> $uniqueTopicId>")
  private val future = retrieveNResultsAsync(10, "src/test/resources/kafkaFilmsJson.shexml", streamLineModification = streamLine).executeAsync.runToFuture
  generateFilmsDataJson.foreach { i =>
    Thread.sleep(5000)
    kafkaEmitter.sendToKafka(i)
  }
