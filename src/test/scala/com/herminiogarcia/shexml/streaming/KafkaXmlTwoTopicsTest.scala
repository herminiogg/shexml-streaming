...
class KafkaXmlTwoTopicsTest extends AnyFunSuite with RDFStatementCreator with ResultRetriever with KafkaTestDataGenerator {

  private val prefix = "http://example.com/"
  private val uniqueTopicId1 = "filmsXml1" + UUID.randomUUID().toString.replaceAll("-", "")
  private val uniqueTopicId2 = "filmsXml2" + UUID.randomUUID().toString.replaceAll("-", "")
  private val kafkaEmitterSet1 = new KafkaTestDataEmitter("localhost:9092", uniqueTopicId1)
  private val kafkaEmitterSet2 = new KafkaTestDataEmitter("localhost:9092", uniqueTopicId2)
  private val streamLine = Some(s"STREAM films_stream <kafka: localhost:9092 -> $uniqueTopicId1, $uniqueTopicId2>")
  private val future = retrieveNResultsAsync(10, "src/test/resources/kafkaFilmsXmlTwoTopics.shexml", streamLineModification = streamLine).executeAsync.runToFuture
  generateFilmsDataXmlSet1.foreach { i =>
    Thread.sleep(5000)
    kafkaEmitterSet1.sendToKafka(i)
  }
  generateFilmsDataXmlSet2.foreach { i =>
    Thread.sleep(5000)
    kafkaEmitterSet2.sendToKafka(i)
  }
  private val output = Await.result(future, 2.minutes)
  ...
  test("Result contains film 10") {
    ...
    assert(output.contains(createStatementWithLiteral(prefix, "10", "year", "1960", XSDDatatype.XSDstring)))
  }
